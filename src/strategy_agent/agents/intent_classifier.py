...
def _instruction(ctx: ReadonlyContext) -> str:
    return (
        "请把用户请求识别为一个量化研究意图。"
        "如果本轮上下文解析显示 rewritten_query，请优先基于 rewritten_query 判断，而不是只看用户当前短句。"
        "如果本轮是 strategy_revision 且上一轮策略已提供排序因子、股票池、持有周期等字段，不要再次把这些继承字段标记为缺失。"
        ...
        f"\n\n{json_contract_instruction(IntentClassificationOutput)}"
    )
